	...
	$(document).ready(function(){
		...
		$('.filter-group').on('click', function(){
			if ( $(this).hasClass('collapsed') ) {
				$(this).children('span:first-child').removeClass('btn-danger');
				$(this).children('span:first-child').addClass('btn-outline-danger');
				$(this).children('span:last-child').removeClass('btn-outline-danger');
				$(this).children('span:last-child').addClass('btn-danger');
			} else {
				$(this).children('span:first-child').addClass('btn-danger');
				$(this).children('span:first-child').removeClass('btn-outline-danger');
				$(this).children('span:last-child').addClass('btn-outline-danger');
				$(this).children('span:last-child').removeClass('btn-danger');
			}
		});
	});
})(jQuery);
	...
	for (let cat of cats) {
		...
		if ( cat.subcats.length > 0 ) {
			for (let subcat of cat.subcats) {
				$('#data').append(`<div class="section-title col m-3 card border border-5 border-${colors[c]}-subtle bg-${colors[c]}-subtle h-100 collapse show" cat="${getCatName(subcat.id)}"><h3 class="fw-bold">${cat.name}: ${subcat.name}</h3></div></div>`);
				getChars(subcat.id, c);
			}
		} else {
			$('#data').append(`<div class="section-title col m-3 card border border-5 border-${colors[c]}-subtle bg-${colors[c]}-subtle h-100 collapse show" cat="${getCatName(cat.id)}""><h3 class="fw-bold">${cat.name}</h3></div></div>`);
			getChars(cat.id, c);
		}
...
function getChars(id, c) {
	for (let obj of data) {
		if ( obj.cat == id ) {
			let output = `<div class="col m-3 card border border-5 border-${colors[c]}-subtle bg-${colors[c]}-subtle h-100 collapse show" cat="${getCatName(id)}">`;
