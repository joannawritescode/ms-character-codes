	...
	for (let cat of cats) {
		...
		if ( cat.subcats.length > 0 ) {
			for (let subcat of cat.subcats) {
				$('#data').append(`<div class="section-title col text-center"><div class="border border-5 border-${colors[c]} bg-${colors[c]}-subtle text-black font-bold"><h3>${cat.name}: ${subcat.name}</h3></div></div>`);
				getChars(subcat.id, c);
			}
		} else {
			$('#data').append(`<div class="section-title col text-center"><div class="border border-5 border-${colors[c]} bg-${colors[c]}-subtle text-black font-bold"><h3>${cat.name}</h3></div></div>`);
			getChars(cat.id, c);
		}
...
function getChars(id, c) {
	for (let obj of data) {
		if ( obj.cat == id ) {
			...
			output += `<td class="keys"><p>PC Alt Keys</p><p>${obj.altkey}</p></td>`;
			output += `<td class="symbol">${obj.symbol}</td>`;
			output += `</tr><tr>`;
			output += `<td colspan="2" class="term">${obj.term}</td>`;
